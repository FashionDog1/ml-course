  ...
  <div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
      ...
      <div class="text-sm text-gray-500 mb-6">
        ...
        <span class="font-medium">课程章节</span>
      </div>

      <!-- Chapter List -->
      <div class="bg-white rounded-lg shadow-md border border-gray-100 p-6">
        <h1 class="text-3xl font-bold mb-6 text-center">课程章节</h1>
        
        <div class="space-y-4">
          <div v-for="(chapter, index) in chapters" :key="chapter.id" class="border-b border-gray-200 pb-4 last:border-b-0 last:pb-0">
            <!-- Chapter Title -->
            <div 
            ...
            >
              <h2 class="text-xl font-semibold flex items-center">
                <span class="bg-blue-100 text-blue-800 rounded-full w-8 h-8 flex items-center justify-center mr-3">{{ index + 1 }}</span>
                {{ chapter.title }}
              </h2>
              <div class="text-gray-500">
                <span v-if="chapter.expanded">▼</span>
                <span v-else>▶</span>
              </div>
            </div>
            ...
            >
              <div 
                ...
                class="py-1"
              >
                <!-- Check if this is a level 2 heading (has subheadings) -->
                <div v-if="subchapter.subchapters && subchapter.subchapters.length > 0" class="mb-2">
                  <div 
                    class="flex justify-between items-center cursor-pointer hover:bg-gray-50 p-2 rounded"
                    @click="toggleSubchapter(chapter.id, subchapter.id)"
                  >
                    <span class="font-medium">{{ subchapter.title }}</span>
                    <div class="text-gray-500">
                      <span v-if="subchapter.expanded">▼</span>
                      <span v-else>▶</span>
                    </div>
                  </div>
                  <!-- Level 3 subchapters -->
                  ...
                  >
                    <router-link 
                      v-for="(subsubchapter, subsubIndex) in subchapter.subchapters" 
                      :key="subsubchapter.id"
                      :to="`/chapters/${chapter.id}/${subsubchapter.id}`"
                      class="block hover:bg-gray-50 p-2 rounded hover:text-blue-600"
                    >
                      {{ subsubchapter.title }}
                    </router-link>
                  </div>
                </div>
                <!-- Regular level 2 subchapter -->
                <router-link 
                  v-else
                  :to="`/chapters/${chapter.id}/${subchapter.id}`"
                  class="block hover:bg-gray-50 p-2 rounded hover:text-blue-600"
                >
                  {{ subchapter.title }}
                </router-link>
